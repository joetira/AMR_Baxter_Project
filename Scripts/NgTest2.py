...
def armsout():
    ...
    zero_zero_right = {'right_s0': 0.0, 'right_s1': 0.00, 'right_w0': 0.00, 'right_w1': 0.00, 'right_w2': 0.00,
                       'right_e0': 0.00, 'right_e1': 0.00}
    zero_zero_left = {'left_s0': 0.0, 'left_s1': 0.00, 'left_w0': 0.00, 'left_w1': 0.00, 'left_w2': 0.00,
                      'left_e0': 0.00, 'left_e1': 0.00}
...
def image_callback(ros_img):
    ...
    image = bridge.imgmsg_to_cv2(ros_img, desired_encoding="passthrough")
    output = image.copy()
    imgGry = cv2.cvtColor(output, cv2.COLOR_BGR2GRAY)

    # mess with threshold values to detect object, depending on lighting etc.
    ret, thrash = cv2.threshold(imgGry, 50, 255, cv2.CHAIN_APPROX_NONE)
    ...
    for contour in cnts:
        ...
        cv2.drawContours(output, [approx], 0, (0, 0, 0), 5)
        x = approx.ravel()[0]
        y = approx.ravel()[1] - 5

        # len(approx) gives sides of object, traingle 3, sqaure 4, pentatgon 5, etc.
        if len(approx) == 3:
            cv2.putText(output, "Triangle", (x, y), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0))
        # use aspect ratio to differntiat b/w sqaure and rectangle, try this with oval/circle?
            ...
            if aspectRatio >= 0.95 and aspectRatio < 1.05:
                cv2.putText(output, "square", (x, y), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0))

            else:
                cv2.putText(output, "rectangle", (x, y), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0))

        elif (len(approx) == 5 or len(approx) == 6):
            cv2.putText(output, "pentagon", (x, y), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0))
        # up length of circle for more "definition" or less if resolution is low maybe
        elif len(approx) == 25:
            cv2.putText(output, "star", (x, y), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0))
        else:
            cv2.putText(output, "circle", (x, y), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0))

    cv2.imshow('Image', np.hstack([image, output]))  # display image
    cv2.waitKey(1)
