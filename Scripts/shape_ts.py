...
def main():
    ...
    initial_poses.append(Pose(
        position=Point(x=0.65, y=-0.35, z=00.129),
        orientation=overhead_orientation))
    initial_poses.append(Pose(
        position=Point(x=-200, y=100, z=0),
        orientation=overhead_orientation))
    # Move to the desired starting angles
    pnp.move_to_start(starting_joint_angles)
    idx=0

    print("Moving to ir calibration orientation")
    pnp._approach(initial_poses[1], hd)

    print("Moving to overhead")
    pnp._approach(initial_poses[idx], hd)
    ...
    while not rospy.is_shutdown():
        ...
        if (center_cond[6]==False and center_cond[7]==False):
            ...
            # temporary for trouble shooting, maybe not e0ven use while loop, not really necessary, its from legacy code
            pnp.gripper_open()
            return 0
