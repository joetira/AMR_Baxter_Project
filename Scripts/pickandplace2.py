...
class PickAndPlace:
    ...
    def image_callback(self, ros_img):
        ...
        colors = ((0, 0, 255), (240, 0, 159), (255, 0, 0), (255, 255, 0))       #for vertices test
        ...
        for (i, c) in enumerate(cnts2):
            ...
            """
            Centroidarray = np.array([xbar,ybar])
            tang = np.subtract(Centroidarray, rect[1])
            xt1 = tang[0]
            yt1 = tang[1]
            x1 = xt1
            y1 = yt1
            ...
            y2 = yt1
            """
            Centroidarray = np.array([self.xbar, self.ybar])
            v1 = rect[0]
            v2 = rect[1]
            v3 = rect[2]
            tri_1 = v3[0]-v1[0]
            ...
            #s1 = np.subtract(v2, v1)
            #s2 = np.subtract(v3, v2)

            #S1 = s1/2
            #S2 = s2/2
            #print s1, s2
            #np.subtract(S1, Centroidarray)
            #np.subtract(S2, Centroidarray)

            #N1 = np.linalg.norm(s1)
            #N2 = np.linalg.norm(s2)
            #print math.degrees(math.atan2(N2, N1))
