...
def get_dice_from_blobs(blobs):
    ...
    if len(X) > 0:
        ...
        for i in range(num_dice):
            ...
            dice.append([len(X_dice), centroid_dice[1:]])
...
class PickAndPlace:
    ...
    def image_callback(self, ros_img):
        ...
        # black
        # lower_hue = np.array([0, 0, 50], np.uint8)
        # upper_hue = np.array([50, 50, 100], np.uint8)
        #
        # black_mask = cv2.inRange(hsv_image, lower_hue, upper_hue)
        ...
        for (i, c) in enumerate(cnts2):
            ...
            #rospy.sleep(1)
            #vertices of side
            S1v = np.subtract(vert2, vert1)
            S2v = np.subtract(vert3, vert2)
            S3v = np.subtract(vert4, vert3)
            S4v = np.subtract(vert1, vert4)
            #length of side
            ...
            unit_anchor = [0, 1]
            new_angles = math.atan2(np.cross(unit_anchor,S1v) ,np.dot(unit_anchor, S1v))

            print("new angle", math.degrees(new_angles))
            print "sides"
            print S1v, S1
            print S2v, S2
            print S3v, S3
            ...
            vert_set = np.vstack([np.subtract(vert1, M), np.subtract(vert2, M), np.subtract(vert3, M), np.subtract(vert4, M)])
            print vert_set
            #delete above
            '''
        ...
        #     cv2.waitKey(500)
        flipBoth = cv2.flip(hsv_image, 0)
        cv2.imshow('real oordinate axis test image  -> (X) ^ (Y)', flipBoth)  # display shape
    ...
    def get_pose(self, z_theta):  # rangeD is global, get rid of this input
        ...
        if None in vert1:
            ...
            print("new angle", math.degrees(gripper_phi))
            #print "sides"
            #print S1v, S1
            #print S2v, S2
            #print S3v, S3
            ...
            QM_adjust = Quaternion(QM_adjust[0].item(), QM_adjust[1].item(), QM_adjust[2].item(), QM_adjust[3].item())
            print "=========="
            print("angle of rotation", math.degrees(gripper_phi))
            print "======="
        ...
        calibrate_offset = np.array([-3.0, 8.25, 0]) / 100.0  #for rectangle, have get data if rectnage based on shape or something
        ...
        return xp, Current_quaternion_msg, new_quaternion, P_camFrame, P_base_gripperFrame_offset, P_baseFrame_calibrate_offset, QM_adjust
