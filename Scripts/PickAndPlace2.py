...
def get_dice_from_blobs(blobs):
    ...
    if len(X) > 0:
        ...
        clustering = cluster.DBSCAN(eps=80, min_samples=0).fit(X)  #was 60 changed at 5/12 1430 b/c dice problems
...
class PickAndPlace:
    ...
    def ik_request(self, pose):
        ...
        ikreq = SolvePositionIKRequest() #msg
    ...
    def roll_die(self, pose, hd, grip_control_param):  #used to have drop pose arg but not used for now
        # open the gripper
        self.gripper_open()
        # servo above pose
        self._approach(pose, hd)
        ...
        print("Updating Gripper Forces", grip_control_param[0])
        self.gripper_close()
        # servo to pose
        #self._servo_to_pose(pose)
        #self.gripper_close()
    ...
    def image_callback(self, ros_img):
        ...
        for (i, c) in enumerate(cnts2):
            ...
            self.v2 = [vert2[0], vert2[1]]
            self.v3 = [vert3[0], vert3[1]]
    ...
    def get_pose(self, z_theta):  # rangeD is global, get rid of this input
        ...
        Current_quaternion_msg = current_pose['orientation']
        ...
        if None in vert1:
            ...
            # vertices of side
            S1v = np.subtract(vert1, vert2)
            S2v = np.subtract(vert2, vert3)
            S3v = np.subtract(vert3, vert4)
            S4v = np.subtract(vert4, vert1)
            # length of side/maybe use for alt method using centroid
            S1 = np.linalg.norm(S1v)
            S2 = np.linalg.norm(S2v)
            S3 = np.linalg.norm(S3v)
            S4 = np.linalg.norm(S4v)
            unit_anchor = [0, 1]
            gripper_phi = math.atan2(np.cross(unit_anchor, S4v), np.dot(unit_anchor, S4v))

            print("estimated orientation change", math.degrees(gripper_phi)), " degrees"
            #print "sides"
            #print S1v, S1
            #print S2v, S2
            #print S3v, S3
            #print S4v, S4
            #print "location of vertices relative to centroid?"
            # order this array like done in order_ponits function
            vert_set = np.vstack(
                [np.subtract(vert1, M), np.subtract(vert2, M), np.subtract(vert3, M), np.subtract(vert4, M)])
            #print vert_set
        ...
        dice_roll_rotation = quaternion_about_axis(z_theta, [0, 1, 0])  #30 degress
        ...
        z1r = new_rotation[2]
        w1r = new_rotation[3]
        dice_roll_orientation = [x1r, y1r, z1r, w1r]
        ...
        QMr = self.quaternion_multiply(current_quaternion, dice_roll_orientation)
        dice_roll_orientation= QMr/np.linalg.norm(QMr)
        dice_roll_orientation = Quaternion(dice_roll_orientation[0].item(), dice_roll_orientation[1].item(), dice_roll_orientation[2].item(), dice_roll_orientation[3].item())
        ...
        P_camFrame = R_GO.dot(camera_vec)  # in gripper coordinates
        ...
        calibrate_offset = np.array([4.0, -7, 0]) / 100.0  #for rectangle, have get data if rectnage based on shape or something
        ...
        P_camFrame = R_BG.dot(P_camFrame)
        P_base_gripperFrame_offset = R_BG.dot(P_gripperFrame_offset)
        P_baseFrame_calibrate_offset = R_BG.dot(P_calibrate_offset)


        # T_BO=np.matmul(T_BG, T_GO)

        return xp, Current_quaternion_msg, new_quaternion, P_camFrame, P_base_gripperFrame_offset, P_baseFrame_calibrate_offset, QM_adjust, dice_roll_orientation
