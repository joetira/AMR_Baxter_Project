...
class PickAndPlace:
    ...
    def image_callback(self, ros_img):
        ...
        gray_img = cv2.cvtColor(cv_image,
                                cv2.COLOR_BGR2GRAY)  # not sure why duplicate, can probably just assign 2nd to 1st var
        ...
        for (i, c) in enumerate(cnts2):
            ...
            cv2.putText(hsv_image, "Object #{}".format(i + 1), (int(rect[0][0] - 15), int(rect[0][1] - 15)),
            ...
            # theta = math.acos((x1*x2+y1*y2)/(math.sqrt(x1*x1+y1*y1)*math.sqrt(x2*x2+y2*y2)))
    ...
    def get_pose(self, z_theta):  # rangeD is global, get rid of this input
        ...
        T_BG = quaternion_matrix(
            current_quaternion)  # gives back rotational homogenous transform matrix from quaternion
        P_BG = [x, y, z, 1]
        ...
        new_quaternion = Quaternion(new_quaternion[0].item(), new_quaternion[1].item(), new_quaternion[2].item(),
                                    new_quaternion[3].item())
        ...
        calibrate_offset = np.array(
            [0.0, 8.25, 0]) / 100.0  # for rectangle, have get data if rectnage based on shape or something
