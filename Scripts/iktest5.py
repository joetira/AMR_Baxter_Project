...
def main():
    ...
    # rospy.Subscriber('/vicon/red_ball/red_ball', TransformStamped, queue_size=10)
    ...
    while not rospy.is_shutdown():
        if not isCentered:
            if CenterOnBall(pnp, idx, hd, block_poses):
                print "Centered, Ready for Pick Up"
                isCentered = True
            else:
                print "Attempting to Center Ball"
        else:
            block_poses[idx].position.x -= 0.05 #######need to rotate
            pnp._approach(block_poses[idx], hd)

            #this while loop isnt' in right spot, idea is to creep close to ball increasing resolution of pick up location
            while rangeD > 0.18:
                hd -= 0.01
                pnp._approach(block_poses[idx], hd)
            ...
            block_poses[idx].position.z = block_poses[idx].position.z+z2pick
            pnp.pick(block_poses[idx], hd)
            pnp.gripper_open()
            exit(1)
        ...
        #     z2pick=gripper_offset-rangeD
        #     #z2pick is negative value so add to z pose below
        #     print block_poses[idx].position.z,block_poses[idx].position.z+z2pick
        #     block_poses[idx].position.z = block_poses[idx].position.z+z2pick
            # pnp.pick(block_poses[idx], hd)
        # raw_input("\nValidate offset to not collide with table\nPress Enter to continue...")
    return 0

#probably can delete all this, pertubation method may be nice for fine tuning location of object however, IK method atm
#isn't exactly too accurate,  for stuff below, may need to rotate camera frame into baxter frame to get accurate results
#only works now b/c its pi/2 offset(almost) to baxter frame
"""      
